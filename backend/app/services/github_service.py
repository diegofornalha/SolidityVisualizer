...
class GitHubService:
    def __init__(self, pat: str | None = None):
        ...
        if (
            ...
            logger.warning("No GitHub credentials provided. Using unauthenticated requests with rate limit of 60 requests/hour.")
    ...
    def _check_repository_exists(self, username, repo):
        ...
        logger.info(f"Checking repository existence: {api_url}")
        ...
        if response.status_code == 404:
            ...
            logger.error(f"Failed to check repository: {response.status_code}, {response.json()}")
            raise Exception(
                f"Failed to check repository: {response.status_code}, {response.json()}"
            )
        logger.info(f"Repository exists: {username}/{repo}")

    def get_default_branch(self, username, repo):
        """Get the default branch of the repository."""
        api_url = f"https://api.github.com/repos/{username}/{repo}"
        logger.info(f"Fetching default branch for: {username}/{repo}")
        response = requests.get(api_url, headers=self._get_headers())

        if response.status_code == 200:
            default_branch = response.json().get("default_branch")
            logger.info(f"Default branch for {username}/{repo}: {default_branch}")
            return default_branch
        logger.warning(f"Could not fetch default branch for {username}/{repo}. Status code: {response.status_code}")
        return None
    ...
    def get_github_file_paths_as_list(self, username, repo):
        ...
        """
        logger.info(f"Attempting to fetch file tree for {username}/{repo}")
        ...
        if branch:
            ...
            logger.info(f"Fetching file tree using default branch {branch}: {api_url}")
            response = requests.get(api_url, headers=self._get_headers())

            if response.status_code == 200:
                data = response.json()
                if "tree" in data:
                    logger.info(f"Successfully fetched file tree using default branch {branch}")
                    paths = [
                        item["path"]
                ...
                logger.warning(f"Failed to fetch file tree using default branch {branch}. Status: {response.status_code}")
                logger.debug(f"Response content: {response.text}")

        # If default branch didn't work or wasn't found, try common branch names
        for branch in ["main", "master"]:
            api_url = f"https://api.github.com/repos/{username}/{repo}/git/trees/{branch}?recursive=1"
            logger.info(f"Trying alternative branch {branch}: {api_url}")
            response = requests.get(api_url, headers=self._get_headers())

            if response.status_code == 200:
                data = response.json()
                if "tree" in data:
                    logger.info(f"Successfully fetched file tree using branch {branch}")
                    paths = [
                        item["path"]
                        for item in data["tree"]
                        if should_include_file(item["path"])
                ...
                logger.warning(f"Failed to fetch file tree using branch {branch}. Status: {response.status_code}")
                logger.debug(f"Response content: {response.text}")

        logger.error(f"Could not fetch repository file tree for {username}/{repo}")
        logger.error("Tried default branch and common branches (main, master)")
        raise ValueError(
    ...
    def get_github_readme(self, username, repo):
        ...
        logger.info(f"Attempting to fetch README for {username}/{repo}")
        ...
        logger.info(f"Fetching README: {api_url}")
        response = requests.get(api_url, headers=self._get_headers())

        if response.status_code == 404:
            logger.error(f"No README found for {username}/{repo}")
            raise ValueError("No README found for the specified repository.")
        elif response.status_code != 200:
            logger.error(f"Failed to fetch README: {response.status_code}, {response.json()}")
            raise Exception(
